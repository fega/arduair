...
function saveDataRequested(res) {
    ...
    function selectPosition(data){
      if (data.isNull()) { 
        ...
        var position = data.checkNewData(name);
        if (position === false || position === null || position === undefined) {
          return data.firstNull();
        }
        if (position === -1) { 
          return 4;
        }
...
function generateChipsForGraph(destiny, data) {
  ...
  $(".page-edit-chip i.material-icons").click(function() { 
      var index = $(this).parent().attr("id").replace('page-edit-chip-', '');
      data.splice(index, 1, null);
      generateGraphMenu(arduair.data,arduair.units); 
  });
}
...
function generateBtnEditGraph(destiny,disabled){
  ...
  $(destiny).append(button);
  if (disabled) $('#page-graph-edit').addClass("disabled"); 

  $('#page-graph-edit').click(() => { 
  ...
  $(content ).appendTo(destiny).bind('click.namespace', function() {
      ...
      $(".filledGraphData").each(function() { 
          ...
          if (oUnits === units) { 
              ...
              if (oValue === value && index !== oIndex && main.hasClass("active")) { 
...
function generateAqiGraphMenu(data){
  data.forEach((item, index) => {
    var ind = index + 1;
    $(`#aqigraph-options-${ind}`).html('');
    var destiny = `#aqigraph-options-${ind}`;
    if (item){
...
function generateAqiGraphMenuBtn(destiny,name,units,device){
  ...
  $(content ).appendTo(destiny).bind('click.namespace', function() {
    ...
    if(active){
      ...
      _(aqiChart.data.datasets).remove({label,device});
    }else{
      $(this).addClass("active");
      if(!_.has(object,normalizedLabel)){
        if(normalizedLabel.includes("instant")){
          arduair.data[objectIndex][normalizedLabel]=arduair.aqi();
        }else{
          arduair.data[objectIndex][normalizedLabel]=arduair.nowcastAqi();
        }
        arduair.data[objectIndex][normalizedLabel]=arduair.nowcastAqi(arduair.data[objectIndex][pollutant],normalizedLabel,arduair.normalizedDates);
      }
      result.label=label;
