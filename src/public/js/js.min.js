...
var arduair = {
  ...
  generateGraphChips() {
      ...
      $('#page-graph-chips').html(content); 

      if (arduair.data.isNull()) $('#page-graph-edit').addClass("disabled"); 
  ...
  nowcastConcentration(arr,pollutant){
    return arr.map((item,index,arr)=>{
      var arrays=[]; 
      var date=moment(arduair.normalizedDates[index]);
...
$(document).ready(() => { 
    ...
    function configSearchSuccess(res) {
        ...
        if (res.status == "done" ) {
          ...
          if(res.device){
            $( "#config-network" ).attr( "value", res.device.network || undefined );
